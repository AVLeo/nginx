...
如果一个用户(这里指浏览器)想鉴别另一个证书的真伪，他就用CA的公钥对那个证书上的签字进行验证，（ca机构会提前把公钥给浏览器厂商）一旦验证通过，该证书就被认为是有效的。
...
```
#####   申请证书过程
![image](https://github.com/chensslinux/nginx/blob/master/ssl/ssl_ca.png)
```
...
#####   SSL工作流程（单向）
![image](https://github.com/chensslinux/nginx/blob/master/ssl/ssl_auth.jpg)
```
1.客户端say hello 服务端
2.服务端将证书、公钥等发给客户端
